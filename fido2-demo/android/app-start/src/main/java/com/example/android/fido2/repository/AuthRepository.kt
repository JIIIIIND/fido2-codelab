...
class AuthRepository(
    ...
    companion object {
        ...
        private var bioResult = BiometricManager.BIOMETRIC_SUCCESS
     ...
     */
    fun signinResponse(data: Intent, processing: MutableLiveData<Boolean>, bioInfo: Int) {
        executor.execute {
            processing.postValue(true)
            try {
                val username = prefs.getString(PREF_USERNAME, null)!!
                ...
                val response = AuthenticatorAssertionResponse.deserializeFromBytes(
                    data.getByteArrayExtra(Fido.FIDO2_KEY_RESPONSE_EXTRA))
                val credentialId = response.keyHandle.toBase64()
                val apiResult = api.signinResponse(sessionId, response)
                prefs.edit(commit = true) {
                    putString(PREF_SESSION_ID, sessionId)
                    putStringSet(PREF_CREDENTIALS, apiResult.data.toStringSet())
                    putString(PREF_LOCAL_CREDENTIAL_ID, credentialId)
                }
                invokeSignInStateListeners(SignInState.SignedIn(username))
                Log.d(TAG, "before $bioResult AND $bioInfo")
                bioResult = bioInfo
                Log.d(TAG, "after $bioResult AND $bioInfo")
